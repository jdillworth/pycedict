...
PINYIN_RE = re.compile(r'(([bcdfghjklmnpqrstwxyz]*)([aeiouv]+)([bcdfghjklmnpqrstwxyz]*)|r)([1-5])', re.I)


TONE_MARKS = {'a':u'_āáǎà', 'e':u'_ēéěè', 'i':u'_īíǐì',
    'o':u'_ōóǒò', 'u':u'_ūúǔù', 'v':u'_ǖǘǚǜ'}
...
ALL_SOUNDS = set(['a', 'ai', 'an', 'ang', 'ao', 'ba', 'bai', 'ban', 'bang',
    ...
    'lun', 'luo', 'lv', 'lvan', 'lve', 'ma', 'mai', 'man', 'mang', 'mao', 'me',
    'mei', 'men', 'meng', 'mi', 'mian', 'miao', 'mie', 'min', 'ming', 'miou',
    'mo', 'mou', 'mu', 'na', 'nai', 'nan', 'nang', 'nao', 'ne', 'nei', 'nen',
    'neng', 'ni', 'nian', 'niang', 'niao', 'nie', 'nin', 'ning', 'niu', 'nong',
    'nou', 'nu', 'nuan', 'nuo', 'nv', 'nve', 'ou', 'pa', 'pai', 'pan', 'pang',
    'pao', 'pei', 'pen', 'peng', 'pi', 'pian', 'piao', 'pie', 'pin', 'ping',
    'po', 'pou', 'pu', 'qi', 'qia', 'qian', 'qiang', 'qiao', 'qie', 'qin',
    'qing', 'qiong', 'qiu', 'qu', 'quan', 'que', 'qun', 'r', 'ran', 'rang',
    ...
    'xun', 'ya', 'yan', 'yang', 'yao', 'ye', 'yi', 'yin', 'ying', 'yong',
...
def pinyinize(src, raise_exception=False):
    ...
    try:
        def replacer(m):
            ...
            try:
                vowels = list(vowels.decode('utf-8'))
    ...
    while i < len(src):
        ...
        if c in unmarked:
            ...
            for p in possible_sounds:
                ...
                before_letter, after_letter = p[:li], p[li+1:]

                if (len(before_letter) == 0 or so_far[-len(before_letter):] == before_letter) and\
                   (len(after_letter) == 0 or lc[i+1:len(after_letter)+i+1] == after_letter):
                    sound = p
                    break
            if sound:
                ...
                # preserve case
                for x in src[i+1:len(after_letter)+i+1]:
                    newstr.append(x)
                newstr.append(u'%d' % tone)
                i += len(after_letter) + 1
            else:
